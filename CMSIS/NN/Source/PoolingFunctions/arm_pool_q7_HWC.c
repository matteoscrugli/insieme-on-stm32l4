...
static void compare_and_replace_if_larger_q7_fixed(q7_t * base,   // base data
                                             q7_t * target, // compare target
                                             const uint16_t length  // data length
    )
{
...
//    while(cnt > 0u)
//    {
//    	if (target[length - cnt] > base[length - cnt])
//    		base[length - cnt] = target[length - cnt];
//    	cnt--;
//    }
...
void
arm_maxpool_q7_HWC_1D(q7_t * Im_in,
                   const uint16_t dim_im_in,
                   const uint16_t ch_im_in,
                   const uint16_t dim_kernel,
                   const uint16_t padding,
                   const uint16_t stride, const uint16_t dim_im_out/*, q7_t * Im_out*/)
{
...
//    for (i_x = 0; i_x < dim_im_out * ch_im_in; i_x++)
    for (i_x = 0; i_x < dim_im_out; i_x++)
    {
        /* for each output pixel */
//        target = Im_in + i_x * ch_im_in;
        q7_t     *target = Im_in + i_x * ch_im_in;
        q7_t     *win_start;
        q7_t     *win_stop;
        ...
        {
            win_start = Im_in + (i_x * stride - padding) * ch_im_in;
        }

        if (i_x * stride - padding + dim_kernel >= dim_im_in)
        {
            win_stop = Im_in + dim_im_in * ch_im_in;
        } else
        {
            win_stop = Im_in + (i_x * stride - padding + dim_kernel) * ch_im_in;
        }
...
//		{
//			BSP_LED_On(0);
//			STLBLE_PRINTF("\r\n%d", dim_im_out * ch_im_in);
//			STLBLE_PRINTF("\r\ntarget: %x, win_start: %x, len: %x", target, win_start, ch_im_in);
////			while(1);
//		}
        /* first step is to copy over initial data */
//        arm_copy_q7(win_start, target, ch_im_in);
        ...
        for (; win_start < win_stop; win_start += ch_im_in)
        {
            compare_and_replace_if_larger_q7_fixed(target, win_start, ch_im_in);
        }
//		while (win_start < win_stop)
//		{
//			compare_and_replace_if_larger_q7_fixed(target, win_start, ch_im_in);
//			win_start += ch_im_in;
//		}
//    	if (i_x == ((44*13) + 10)) BSP_LED_On(0);
    }
//    memcpy(Im_out, Im_in, dim_im_out * ch_im_in);
}
